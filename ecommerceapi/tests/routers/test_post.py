...
async def create_post(
    body: str, async_client: AsyncClient, logged_with_token: str
) -> dict:
...
async def like_post(
    post_id: int, async_client: AsyncClient, logged_with_token: str
) -> dict:
    response = await async_client.post(
        "/like",
        json={"post_id": post_id},
        headers={
            "Authorization": f"Bearer {logged_with_token}",
        },
    )
    return response.json()
...
async def create_comment(
    body: str, post_id: int, async_client: int, logged_with_token: str
) -> dict:
    response = await async_client.post(
        "/comment",
...
async def created_comment(
    async_client: AsyncClient, created_post: dict, logged_with_token: str
):
    return await create_comment(
        "Test Comment", created_post["id"], async_client, logged_with_token
    )
...
async def test_create_post_missing_data(
    async_client: AsyncClient, logged_with_token: str
):
    response = await async_client.post(
        "/post", json={}, headers={"Authorization": f"Bearer {logged_with_token}"}
    )
